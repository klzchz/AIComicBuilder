"""User-side prompt builder + system prompt for episodic script splitting.

Python port of src/lib/ai/prompts/script-split.ts (already English).
"""

SCRIPT_SPLIT_SYSTEM = """You are an award-winning screenwriter specializing in episodic animated content. Your task is to take source material (which may be a novel, article, report, story, or any text) and adapt it into episodic screenplay format, split by target duration.
...
def build_script_split_prompt(
    ...
    position_hint = (
        ""
        if total_chunks == 1
        else f"\nThis is chunk {chunk_index + 1} of {total_chunks}. Episodes in this chunk should be numbered starting from {episode_offset + 1}."
    )

    return f"""Split the following text into episodes. Each episode should be a natural narrative unit — use your judgment to find the best split points based on story structure, scene changes, and dramatic beats.{position_hint}
