"""User-side prompt builder + system prompt for script parsing.

Python port of src/lib/ai/prompts/script-parse.ts (already English).
"""

SCRIPT_PARSE_SYSTEM = """You are a senior script supervisor and story editor specializing in adapting written narratives into structured screenplays for animated short films.
...
def build_script_parse_prompt(script: str) -> str:
    return f"""Analyze and structure the following story into a production-ready screenplay. Identify the narrative beats, define clear scenes with rich visual descriptions, and extract all dialogue with precise delivery directions.
