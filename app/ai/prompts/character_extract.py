"""User-side prompt builder + system prompt for character extraction.

Python port of src/lib/ai/prompts/character-extract.ts (already English).
"""
...
def build_character_extract_prompt(screenplay: str) -> str:
    return f"""Extract and create detailed visual character specifications for EVERY named character in this screenplay. Each description must be specific enough to serve as a binding art reference for consistent AI image generation.
